...
import { revealGroup, revealItem, revealTransition } from '../lib/motion';
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-5 md:gap-7">
        <motion.div variants={revealItem} className={cn('max-w-4xl space-y-3 md:space-y-5', isArabic ? 'text-right' : 'text-left')}>
        ...
        {displayMetrics?.length ? (
          ...
          >
            {displayMetrics.map((metric) => (
              ...
              >
                <p className="font-display text-base md:text-2xl lg:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-l from-cyan-300 to-teal-300">
                  {metric.value}
                </p>
